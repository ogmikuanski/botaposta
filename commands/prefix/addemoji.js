...
module.exports = {
  ...
  execute: async (message, args, client) => {
    ...
    try {
      ...
      console.error("[PREFIX ADDEMOJI]: " + err);
      
      let errorMsg = process.env.MSGERROBOT || "Ocorreu um erro desconhecido.";
      
      if (err.code === 50035) errorMsg = "Link inválido, imagem corrompida ou nome inválido.";
      if (err.code === 30008) errorMsg = "O servidor atingiu o limite máximo de emojis.";
      if (err.code === 50013) errorMsg = "Permissões insuficientes (verifique se meu cargo está acima).";
      if (err.code === 40005) errorMsg = "O arquivo de imagem é muito grande (max 256kb).";
